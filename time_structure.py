...
class TimeLine(List):
    ...
    def print(self, save=False):
        ...
        if save:
            macrotimesteps_txt = open("macrotimesteps.txt", "a")
            microtimesteps_txt = open("microtimesteps.txt", "a")
        for n in range(global_size):

            print(f"Macro time-step size: {macrotimestep.size}")
            microtimestep = macrotimestep.head
            macrotimesteps_txt.write(f"{microtimestep.point} ")
            ...
            for m in range(local_size):
                ...
                if m != local_size - 1:
                    microtimesteps_txt.write(f"{microtimestep.point} ")
        ...
        if save:
            macrotimesteps_txt.write(f"{microtimestep.point} ")
            macrotimesteps_txt.close()
            microtimesteps_txt.write(f"{microtimestep.point} ")
            microtimesteps_txt.close()

    # Create a macrotimestep consisting of equidistant microtimesteps
    def unify(self, time_step_size, local_size, global_size):

        initial = MacroTimeStep()
        initial.unify(0.0, time_step_size / local_size, local_size)
        self.head = initial
        element = self.head
    ...
    def load(self, space: Space, space_name, adjoint):
        ...
        if not adjoint:
            ...
            velocity = Initial(
                space_name, "primal_velocity", space.function_space_split[1]
            )
            ...
            velocity_adjoint = Initial(
                space_name, "adjoint_velocity", space.function_space_split[1]
            )
        ...
        for n in range(global_size):
            ...
            for m in range(local_size):
                ...
                if not adjoint:
                    ...
                    microtimestep.functions[
                        "adjoint_displacement"
                    ] = displacement_adjoint.load(counter)
                    microtimestep.functions[
                        "adjoint_velocity"
                    ] = velocity_adjoint.load(counter)
