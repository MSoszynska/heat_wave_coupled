def refine(
    ...
    fluid_residuals = [
        abs(x[0]) + abs(x[1]) for x in zip(primal_fluid, adjoint_fluid)
    ]
    solid_residuals = [
        abs(x[0]) + abs(x[1]) for x in zip(primal_solid, adjoint_solid)
    ]
    average = sum([abs(x) for x in fluid_residuals]) / (
        2 * len(fluid_residuals)
    ) + sum([abs(x) for x in solid_residuals]) / (2 * len(solid_residuals))
    fluid_refinements = []
    solid_refinements = []
    for i in range(len(fluid_residuals)):

        if (abs(primal_fluid[i]) > average) or (
            abs(adjoint_fluid[i]) > average
        ):
    ...
    for i in range(len(solid_residuals)):
        ...
        if (abs(primal_solid[i]) > average) or (
            abs(adjoint_solid[i]) > average
        ):
